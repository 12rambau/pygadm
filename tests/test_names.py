...
def test_duplicate_input():
    ...
    with pytest.raises(Exception):
        pygadm.Names(name="Singapore", admin="SGP")
...
def test_area(dataframe_regression):
    ...
    df_admin = pygadm.Names(admin="SGP")
    assert df_admin.equals(df)


def test_sub_content(dataframe_regression):
    """Request a sublevel."""
    df = pygadm.Names(name="Singapore", content_level=1)
    dataframe_regression.check(df)
...
def test_too_high(dataframe_regression):
    ...
    with pytest.warns(UserWarning):
        df = pygadm.Names(admin="SGP.1_1", content_level=0)
        dataframe_regression.check(df)
...
def test_too_low(dataframe_regression):
    ...
    with pytest.warns(UserWarning):
        df = pygadm.Names(admin="SGP.1_1", content_level=3)
        dataframe_regression.check(df)
